...
int dw_main(void)
{
    ...
    * As this example only handles one incoming frame with always the same delay and timeout, those values can be set here once for all. */
    ...
    while (1)
    {
        tx_poll_msg[5] = responder_addresses[2 * current_responder];
        tx_poll_msg[6] = responder_addresses[2 * current_responder + 1];
        rx_resp_msg[7] = responder_addresses[2 * current_responder];
        rx_resp_msg[8] = responder_addresses[2 * current_responder + 1];
        tx_final_msg[5] = responder_addresses[2 * current_responder];
        tx_final_msg[6] = responder_addresses[2 * current_responder + 1];

        current_responder = (current_responder + 1) % TOTAL_RESPONDERS;
