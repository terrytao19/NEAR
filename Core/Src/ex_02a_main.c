...
int dw_main(void)
{
    ...
    while (1)
    {
        ...
        uint32_t status_reg_error = status_reg & (SYS_STATUS_RXFCG | SYS_STATUS_ALL_RX_ERR);
        // uint32_t sys_cfg = dwt_read32bitreg(0x04);
        // CDC_Transmit_FS((uint8_t *) &sys_cfg, sizeof(sys_cfg));
        // CDC_Transmit_FS((uint8_t *) &status_reg_error, sizeof(status_reg_error));
        CDC_Transmit_FS(rx_buffer, FRAME_LEN_MAX);
    }
}
#endif
/*****************************************************************************************************************************************************
